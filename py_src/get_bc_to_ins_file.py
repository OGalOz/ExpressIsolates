...
def get_bc_to_read_dict(bc_fasta_fp):
    ...
    while read_line != "":
        ...
        if len(BC_seq) != 20:
            raise Exception(f"Expecting bc to be length 20, line no. {line_num}, " + \
...
def get_read_to_seq_dict_from_fq(fq_fp):
    ...
    while read_line != "":
        if read_line[0] != "@":
            raise Exception(f"Expecting '@' at first pos, " + \
                            f"line no. {line_num}, file {fq_fp}")
        read_name = read_line.rstrip()[1:]
        seq_line = FH.readline().rstrip()
        spacer_line = FH.readline().rstrip()
        if spacer_line != "+":
            raise Exception(f"Expecting '+' at " + \
                            f"line no. {line_num + 2}, file {fq_fp}")
        qual_line = FH.readline().rstrip()
        if len(seq_line) != len(qual_line):
            raise Exception("Expecting length of seq and qual to be " + \
                            f"the same at lines {line_num + 1}, {line_num + 3}")
        if read_name in read2seq:
            raise Exception(f"Found duplicate read name in fq file: {fq_fp}," + \
                            f" line no. {line_num}")
        else:
